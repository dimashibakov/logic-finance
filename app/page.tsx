...
export default async function Home() {
  ...
  const top = [...accounts]
    .sort((a, b) => Math.abs(toUsd(Number(b.balance), b.currency, rubPerUsd)) - Math.abs(toUsd(Number(a.balance), a.currency, rubPerUsd)))
    .slice(0, 6);
  ...
  return (
    <div style={S.wrap}>
      <div style={S.phone}>
        <div style={S.header}>
          <span style={S.title}>PORTFOLIO · Logic Finance</span>
        ...
        <div style={{ ...S.mono, fontSize: 12, color: C.sub, marginTop: 4 }}>assets {usd(assets)} − debt {usd(debt)}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, margin: "16px 0" }}>
          ...
          <div style={S.card}>
            ...
            <div style={{ ...S.mono, fontSize: 11, color: usdZone >= 0 ? C.up : C.down }}>cash + cards</div>
          </div>
        </div>

        <div style={{ ...S.label, marginBottom: 8 }}>ACCOUNTS & ASSETS</div>
        <div style={{ ...S.card, padding: 0 }}>
          {top.map((a, i) => {
            const bal = Number(a.balance);
            const native = a.currency === "USD" ? usd(bal) : rub(bal);
            const usdVal = usd(toUsd(bal, a.currency, rubPerUsd));
            return (
              <div
                key={i}
                style={{
                  ...
                  alignItems: "center",
                  padding: "12px 14px",
                  borderBottom: i < top.length - 1 ? `1px solid ${C.line}` : "none",
                }}
              >
                <div>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ ...S.mono, fontSize: 13, fontWeight: 600, color: bal < 0 ? C.down : C.ink }}>{native}</div>
                  <div style={{ ...S.mono, fontSize: 11, color: C.faint, marginTop: 2 }}>{usdVal}</div>
                </div>
              </div>
            );
          })}
          {top.length === 0 && (
            <div style={{ padding: 16, fontSize: 13, color: C.sub }}>No data — check Supabase connection (.env.local).</div>
          )}
        </div>
        ...
        <div style={{ ...S.card, marginTop: 12, background: "#F0F4FF", borderColor: "#1652F022", display: "flex", gap: 10 }}>
          ...
          <div style={{ fontSize: 12.5, color: C.ink, lineHeight: 1.5 }}>
            Live data from Supabase ({accounts.length} accounts, {obligations.length} obligations). Use the tabs below for Rates, Convert, Debts, and Plan.
          </div>
        </div>
      </div>
    </div>
